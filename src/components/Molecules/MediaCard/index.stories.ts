...
storiesOf('Molecules/MediaCard', module)
  ...
  .add('基本情報', withNotes(
    ...
    return {
      ...
      template: `<MediaCard title="${ title }" description="${ description }" image="${ image }" url="${ url }" alt="${ alt }" />`,
      propsDescription: {
        url: `画像のURL`,
        alt: '画像のAltテキスト',
        title: '画像のタイトルテキスト',
...
storiesOf('Molecules/MediaCard', module)
  .add('Normal', () => ({
    components: { MediaCard },
    template: '<MediaCard image="https://jungle-time.com/wp-content/uploads/2018/03/red-fox-2230730_1280.jpg" url="index.html" alt="Fox Image" width="400" height="240" />',
